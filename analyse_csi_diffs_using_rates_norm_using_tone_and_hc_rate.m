...
%% make 16 x 6 rates normalised 
rates_and_sig = set1_rates_and_sig;
for u=1:299
    u_rates = rates_and_sig{u,2};
    first_tone_rates = u_rates([1:4 13:16],1);
    first_hc_rates = u_rates([5:8 9:12],1);

    sd_tone = nanstd(first_tone_rates);
    sd_hc = nanstd(first_hc_rates);

    norm_fac = (sd_tone^2 + sd_hc^2)^0.5;

    u_rates_norm = u_rates./norm_fac;
    rates_and_sig{u,9} = u_rates_norm;
end
set1_rates_and_sig_norm = rates_and_sig;
save('set1_rates_and_sig_norm_by_t_hc.mat', 'set1_rates_and_sig_norm')
...
set2_rates_and_sig_norm = rates_and_sig;
save('set2_rates_and_sig_norm_by_t_hc.mat', 'set2_rates_and_sig_norm')
%% load norm rates
set1_rates_and_sig_norm = load('set1_rates_and_sig_norm_by_t_hc.mat');
set1_rates_and_sig_norm = set1_rates_and_sig_norm.set1_rates_and_sig_norm;

set2_rates_and_sig_norm = load('set2_rates_and_sig_norm_by_t_hc.mat');
set2_rates_and_sig_norm = set2_rates_and_sig_norm.set2_rates_and_sig_norm;


%% make 4 d cell with norm rates instead of normal rates
gap_values = [60 90 110 130 150 280];
...
rates_and_sig = set1_rates_and_sig_norm;
...
for u=1:299
    ...
    sig_vals = reshape(sig_vals(:,1:3), numel(sig_vals(:,1:3)),1);
...
rates_and_sig = set2_rates_and_sig_norm;
...
figure
   ...
   hold off
    ...
    title(strcat(string(stim1),'-vs-',string(stim2),'-seg-', num2str(seg), '-is-it-csi-',num2str(is_it_csi)))
grid
