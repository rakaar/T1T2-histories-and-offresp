    ...
    response_struct = load(response_file).unit_record_spike;
     ...
     for u=1:length(response_struct)
        ...
        for iter=1:80
             ...
             stim_spont(1,iter) = sum(spikes_from_negspike_timings(1, starting_pt-(stim_durn + off_res_comp)+1:starting_pt));  
        ...
        for s=1:n_stimulus
            ...
            single_unit_sig_matrix(s,1) = ttest2(stim_spont, sum(responses_for_single_stim(:, token1_start_time:token1_end_time + off_res_comp), 2), 'Alpha',alpha/bonferoni_factor);  
            single_unit_sig_matrix(s,2) = ttest2(stim_spont, sum(responses_for_single_stim(:, token2_start_time:token2_end_time + off_res_comp), 2), 'Alpha',alpha/bonferoni_factor); 
            single_unit_sig_matrix(s,3) = ttest2(stim_spont, sum(responses_for_single_stim(:, token3_start_time:token3_end_time + off_res_comp), 2), 'Alpha',alpha/bonferoni_factor);
...
% fill all empty braces with zeros
for u=1:decent_num
    for s=1:n_stimulus
        if isempty(only_sig_units_db{u,s})
            only_sig_units_db{u,s} = zeros(1,total_stimulus_duration);
        end
    end
end
...
for u=1:n_units
    ...
    for s=1:n_stimulus
        ...
        unit_response_for_stim = cell_to_be_analysed{u,s};
        n_rep = size(unit_response_for_stim, 1);

        for n=1:n_rep
            single_rep_single_unit_single_stim_res = unit_response_for_stim(n,:);
            % stim tokens
            single_unit_all_stim_6segement_values(s,n,1) = mean(unit_response_for_stim(1, token1_start_time:token1_end_time+off_res_comp));
            single_unit_all_stim_6segement_values(s,n,2) = mean(unit_response_for_stim(1, token2_start_time:token2_end_time+off_res_comp));
            single_unit_all_stim_6segement_values(s,n,3) = mean(unit_response_for_stim(1, token3_start_time:token3_end_time+off_res_comp));
...
%% mean % sig or all - based on above
for u=1:299 % sig
    mean_all_units{u,1} = squeeze(nanmean(mean_all_units{u,1}, 2));
end

%% sig values matrix
for u=1:299 % sig
    mean_all_units{u,2} = units_sig_matrix{u,1};
end
%% only sig units
only_sig_sig_matrix = cell(decent_num, 2);
u_counter = 1;
for u=1:299
    if units_sig_matrix{u,2} == 1
        only_sig_sig_matrix{u_counter,2} = units_sig_matrix{u_counter,1};
        u_counter = u_counter + 1;
    end
end

for u=1:187 
    only_sig_sig_matrix{u,1} = mean_all_units{u,1};
end
