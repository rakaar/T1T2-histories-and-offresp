...
% 1 - rate 16 stim x 5 iters x 6 segments
...
for u=1:n_units
    single_unit_all_stim_6segement_values = nan(n_stimulus, 5, 6); % 16 stim, max 5 rep of each stim, 6 segments
    ...
    for s=1:n_stimulus
        ...
        for n=1:n_rep
            ...
            % gaps
            single_unit_all_stim_6segement_values(s,n,4) = nanmean(single_rep_single_unit_single_stim_res(1, gap1_start_time+off_res_comp:gap1_end_time));
            single_unit_all_stim_6segement_values(s,n,5) = nanmean(single_rep_single_unit_single_stim_res(1, gap2_start_time+off_res_comp:gap2_end_time));
            single_unit_all_stim_6segement_values(s,n,6) = nanmean(single_rep_single_unit_single_stim_res(1, gap3_start_time+off_res_comp:gap3_end_time));
...
stim_durn = 50; gap1_durn = gaps(1); gap2_durn = gaps(2); gap3_durn = gaps(3); gap4_durn = gaps(4);
starting_pt = 500;
off_res_comp = 20;
    ...
    response_struct = load(response_file).unit_record_spike;
     ...
     for u=1:length(response_struct)
        ...
        for iter=1:80
             ...
             gap1_spont(1,iter) = nanmean(spikes_from_negspike_timings(1, starting_pt-gap1_durn+off_res_comp+1:starting_pt));  
             gap2_spont(1,iter) = nanmean(spikes_from_negspike_timings(1, starting_pt-gap2_durn+off_res_comp+1:starting_pt));  
             gap3_spont(1,iter) = nanmean(spikes_from_negspike_timings(1, starting_pt-gap3_durn+off_res_comp+1:starting_pt));  
             gap4_spont(1,iter) = nanmean(spikes_from_negspike_timings(1, starting_pt-gap4_durn+off_res_comp+1:starting_pt));  
        end
        ...
        for s=1:n_stimulus
            ...
            if n_res < 4
                ...
                disp('never here for second set bf 16')
            else
             gap_type = mod(s,4);
            if gap_type == 1
                gap_durn = gap1_durn;
            ...
            % gaps
            mean4 = nanmean(responses_for_single_stim(:, gap1_start_time+off_res_comp:gap1_end_time), 2);
            mean5 = nanmean(responses_for_single_stim(:, gap2_start_time+off_res_comp:gap2_end_time), 2);
            mean6 = nanmean(responses_for_single_stim(:, gap3_start_time+off_res_comp:gap3_end_time), 2);
